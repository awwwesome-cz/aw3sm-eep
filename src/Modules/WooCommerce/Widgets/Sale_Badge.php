...
class Sale_Badge extends Widget_Base {
	public function __construct( $data = [], $args = null ) {
		...
		wp_register_style( 'sale-badge-stylesheet', plugin_dir_url( __FILE__ ) . '/../../css/sale-badge.woocommerce.css' );
	}

	public function get_style_depends() {
		return [ 'sale-badge-stylesheet' ];
	}
	...
	protected function register_controls() {
		...
		do_action( 'aw3sm_eep/widgets/woocommerce/sale-badge/template/before_section_end', $this );

		$this->end_controls_section();

		do_action( 'aw3sm_eep/widgets/woocommerce/sale-badge/template/after_section_end', $this );
	...
	protected function render() {
		...
		// if not in sale
		// fix null while saving widget on elementor
		...
		if ( $value_type == 'percentage' ) {
			...
			echo apply_filters( 'woocommerce_sale_flash',
				'<span class="aw3sm-woo-onsale-badge">' . __( 'Sale!', 'woocommerce' ) . '</span>',
				$post,
				$product );
		}
	...
	protected function display_percentage_on_sale_badge( $product ) {
		...
		if ( $product->is_type( 'variable' ) ) {
			...
			if ( $sale_price != 0 || ! empty( $sale_price ) ) {
				$percentage = $prefix_sale . round( 100 - ( $sale_price / $regular_price * 100 ) ) . $suffix_sale;
			} else {
				$percentage = $prefix_sale . '0' . $suffix_sale;
			}
		}
